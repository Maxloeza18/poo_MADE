...
"""
Basado en este script desarrollar la practica 01 de la asignatura:
...
def mostrar_interfaz_hmi(actuadores, sensores):
    ...
    print("   • ajustar <actuador> <val>  (Ej: ajustar ventilador 75.5)")
    print("   • leer <sensor>             (Ej: leer caudal / leer temperatura)")
...
# ==============================================================================
def main():
    # 3. Creación de tres objetos de la clase Actuador
    bomba = Actuador("BOMBA DE AGUA")
    valvula = Actuador("VALVULA DE CONTROL")
    ventilador = Actuador("VENTILADORDOR DE REFRIGERACION")

    # 3. Creación de tres objetos de la clase Sensor
    ...
    sensores = {"caudal": medidor_caudal, "manometro": manometro, "temperatura": sensor_temperatura}
    ...
    while True:
        ...
        if comando == "encender":
            if len(partes) < 2:
                registrar_evento("[⚠️ ERROR] Especifica el actuador. Uso: encender <bomba/valvula/ventilador>")
                continue
        ...
        # Procesamiento del Comando: APAGAR
            ...
            if len(partes) < 2:
                registrar_evento("[⚠️ ERROR] Especifica el actuador. Uso: apagar <bomba/valvula/ventilador>")
                continue
            target = partes[1].lower()
            if target in actuadores:
        ...
        # Procesamiento del Comando: AJUSTAR
            ...
            if len(partes) < 3:
                registrar_evento("[⚠️ ERROR] Faltan parámetros. Uso: ajustar <bomba/valvula/ventilador> <valor>")
                continue
            target = partes[1].lower()
            try:
        ...
        # Procesamiento del Comando: LEER
            ...
            if len(partes) < 2:
                registrar_evento("[⚠️ ERROR] Especifica el sensor. Uso: leer <caudal/manometro/temperatura>")
                continue
            target = partes[1].lower()
